...
class GreedyHillClimbExplorer(Explorer):
    ...
    def seedScore(self, result: float) -> None:
        ...
        if self.index < len(self.techniques):
            ...
            self.scores.append((self.transforms[self.index], result))
            # Handle the selection and transform generation on the last iteration of the single techniques
            if self.index == len(self.techniques) - 1 and self.transform_length <= self.TRANSFORM_LENGTH_CAP:
                top_10_transform_scores = sorted(self.scores, lambda x: x[1], reverse=True)[:10]
    ...
    def __generateTransformsFromScores(self) -> None:
        top_10_transform_scores = sorted(self.scores, lambda x: x[1], reverse=True)[:10]
        new_transforms = self.__incrementTransformsLength([tuple[0] for tuple in top_10_transform_scores])
        
        self.transforms.extend(new_transforms)

    def __incrementTransformsLength(self, transforms: list[Transform]):
        new_transforms = []
        ...
        for transform in transforms:
            ...
            for technique in self.techniques:
                if technique not in techniques:
                    new_transforms.append(Transform(techniques.append(technique)))
